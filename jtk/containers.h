...
namespace jtk
  ...
  class vector_t
    ...
    public:
      ...
      inline       iterator end() { return items + size_active; };
      inline const_iterator end() const { return items + size_active; };


      /********************** Capacity ****************************/

      inline bool   empty() const { return size_active == 0; }
      inline size_t size() const { return size_active; }
      inline size_t capacity() const { return size_alloced; }
      ...
      inline       T& operator[](size_t i) { assert(i < size_active); return items[i]; }
      inline const T& operator[](size_t i) const { assert(i < size_active); return items[i]; }

      inline       T& at(size_t i) { assert(i < size_active); return items[i]; }
      inline const T& at(size_t i) const { assert(i < size_active); return items[i]; }

      inline T& front() const { assert(size_active > 0); return items[0]; };
      inline T& back() const { assert(size_active > 0); return items[size_active - 1]; };

      inline       T* data() { return items; };
      inline const T* data() const { return items; };
